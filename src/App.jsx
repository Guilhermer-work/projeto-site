...
if (isMestre) {
  // sempre busca do backend para garantir dados atualizados
  apiFetch(`/fichas/${fichaObj.id}`)
    .then(res => {
      if (!res.ok) throw new Error("Erro ao buscar ficha do jogador");
      return res.json();
    })
    .then(data => {
      setFichaVisualizada({
      ...
      setSomenteVisualizar(true); // mestre só vê, não salva em personagens
    })
    .catch(err => {
      console.error("Erro ao carregar ficha do jogador para o mestre:", err);
    });
